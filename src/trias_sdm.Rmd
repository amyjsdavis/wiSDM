...
summary(GlobalModelResults)# displays accuracy of each model
...
#option 1. stack all available climate data (default)
...
```
#option 2. stack selected climate data
```{r stack selected_euclimate_preds}  

# delete or comment any variables that should be excluded from the raster stack
climate.vars<-list("anngdd100.tif",
                         "annprecip_eea.tif",
                         "annpvarrecip_eea.tif",
                         "anntemp_eea.tif",
                         "dristprec.tif",
                         "maxtemp.tif",        
                         "mintemp.tif",
...
climate.vars.selected<-paste0(here("./data/external/climate/rmi_corrected"),"/", climate.vars)
rmiclimpreds<-stack(climate.vars) 
...
EU_ModelResults1<-resamples(model_train_habitat)
summary(EU_ModelResults1)
...
EU_Ens_ModelResults1<-resamples(model_train_habitat1)
summary(EU_Ens_ModelResults1)
...
```{r Plot_ensModel_eu}
  ...
  plot(ens_pred_hab_eu1, breaks=brks, col=cols,lab.breaks=brks)
   plot(euocc1,pch=1,cex=0.50,add=TRUE)#plots species presences comment this line to hide
  plot(euocc_pseudoAbs,pch=2,cex=0.50,add=TRUE)#plots species pseudoabsences comment this line to hide
...
### Code for conformal prediction functions
...
hist.conf.thresh.map<-confidenceMaps(pvalsdf_hist,taxonkey,taxonName,maptype="hist_conf_thresh")
rcp26.conf.thresh.map<-confidenceMaps(pvalsdf_rcp26,taxonkey,taxonName,maptype="rcp26_conf_thresh")
rcp45.conf.thresh.map<-confidenceMaps(pvalsdf_rcp45,taxonkey,taxonName,maptype="rcp45_conf_thresh")
rcp85.conf.thresh.map<-confidenceMaps(pvalsdf_rcp85,taxonkey,taxonName,maptype="rcp85_conf_thresh")
```


### Model evalution summary
```{r export ModelEvaluation}
meanResults<-summary(EU_Ens_ModelResults1)
